...
def load_data(world_rank, world_size, data_tr_cpu, label_tr_cpu):
    ...
    dataloader = []
    n_split = 300 # 60000/400=150 # 1000
    tmp_skf = StratifiedKFold(n_splits=n_split, shuffle=True, random_state=0) # for minibatch training with shuffling the samples
...
# === データロード関数 (DDP対応) ===
def load_distributed_data(rank, world_size, data_tr_cpu, label_tr_cpu, batch_size):
    dataset = TensorDataset(data_tr_cpu, label_tr_cpu)
    # DistributedSampler: 各プロセスにデータの一部を割り当てる
    sampler = DistributedSampler(dataset, num_replicas=world_size, rank=rank, shuffle=True) # 訓練時は shuffle=True
    # DataLoader: Samplerを使ってミニバッチを作成
    # pin_memory=True で CPU->GPU転送を高速化
    dataloader = DataLoader(dataset, batch_size=batch_size, sampler=sampler, pin_memory=True, num_workers=0) # num_workers=0 for simplicity
    return dataloader
...
def run(world_rank, world_size, data_tr_cpu, label_tr_cpu):
    ...
    # mini_batch_size = 150 # 60000/400=150
    #dataloader = load_distributed_data(world_rank, world_size, data_tr_cpu, label_tr_cpu, mini_batch_size)
